...
//mobile_prefix
if (!function_exists('mobile_prefix')) {
...
if (!function_exists('parcelStatus')) {
    function parcelStatus($parcel, $request = null)
    {
        ...
        } elseif ($parcel->status == ParcelStatus::COLLECT_TO_PICKUP) {
            $allowStatus = [
...
if (!function_exists('StatusParcel')) {
    function StatusParcel($status_id)
    {
        if ($status_id == ParcelStatus::PENDING) {
            ...
            $status = '<span class="badge badge-pill badge-info">' . trans("parcelStatus." . $status_id) . '</span>';
        } elseif ($status_id == ParcelStatus::BRANCH_CANCEL) {
            $status = '<span class="badge badge-pill badge-warning">' . trans("parcelStatus." . $status_id) . '</span>';
...
if (!function_exists('notifications')) {
    ...
    function notifications()
    {
        ...
        $supports          = Support::whereNot('user_id', Auth::user()->id)
            ->orderBy('created_at', 'DESC')
            ->select('id', 'user_id', 'subject', 'created_at')
            ->whereBetween('created_at', [$sevendaysBefore, $today])
            ->get();
        ...
        $supportsChats          = SupportChat::orderBy('created_at', 'DESC')
