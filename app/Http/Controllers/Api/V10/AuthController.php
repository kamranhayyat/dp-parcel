...
use App\Http\Requests\Merchant\CbSignUpRequest;
...
class AuthController extends Controller
{
    ...
    public function otpVerification(OtpRequest $request)
    {
        ...
        if ($result != null) {
            ...
            if ($user) {
                Auth::login($user);
                return $this->responseWithSuccess(__('auth.signin_msg'), ['token' => $user->createToken($result->mobile)->plainTextToken, 'user' => new UserResource(auth()->user())], 200);
            } else {
    ...
    {
        $validator = new CbSignUpRequest();
        $validator = Validator::make($request->all(), $validator->rules());
        if ($validator->fails()) {
            return $this->responseWithError(__('merchant.added_msg'), ['message' => $validator->errors()], 422);
        }
        $request['password'] = Hash::make($request->mobile);
        if ($this->merchantRepo->signUpStore($request)) {
            return $this->responseWithSuccess(__('merchant.added_msg'), ['mobile' => $request->mobile], 200);
        } else {
            return $this->responseWithError(__('merchant.error_msg'), [], 500);
    ...
    public function cbLogin(Request $request)
    {
        ...
        if ($user) {
            if ($this->merchantRepo->sendOTP($request)) {
                return $this->responseWithSuccess(__('auth.send_otp_msg'), [
                    'mobile' => $request->mobile
                ], 200);
            } else {
                return $this->responseWithError(__('auth.error_msg'), [], 500);
            ...
            return $this->responseWithError(__('auth.invalid_mobile'), [], 500);
        }
    }
    ...
    public function signin(Request $request)
    {
        ...
        return $this->responseWithSuccess(__('auth.signin_msg'), [
    ...
    public function deliveryManLogin(Request $request)
    {
        ...
        $user = User::find(auth()->id());
        return $this->responseWithSuccess(__('auth.signin_msg'), ['token' => $user->createToken($request->email)->plainTextToken, 'user' => new  DeliverymanUserResource(auth()->user())], 200);
    }
