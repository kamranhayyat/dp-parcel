...
class DeliverymanController extends Controller
{
    ...
    public function dashboard()
    {
        try {
            $data         = [];
            $data['pickup_assign']      = ParcelResource::collection($this->parcel->deliverymanStatusParcel(ParcelStatus::PICKUP_ASSIGN));
    ...
    public function profile()
    {
        try {
            ...
            $data['total_delivery_charge'] = (string) $this->deliveryman->totalDeliveryCharge();
            $data['deliveryman_earn']      = (string) $this->deliveryman->deliverymanEarn(StatementType::INCOME)->whereNull('income_id')->whereNotNull('parcel_id')->where('cash_collection', 0)->sum('amount');
            $cod = $this->deliveryman->totalCOD(StatementType::EXPENSE)->sum('amount') - $this->deliveryman->totalCOD(StatementType::INCOME)->sum('amount');
            $data['total_cod']             =  (string)$cod;
    ...
    public function parcelStatusUpdate(Request $request)
    {
        switch ($request->status_action) {
                //Pickup Collected
            case ParcelStatus::COLLECT_TO_PICKUP:
                if ($this->parcel->collectToPickup($request->parcel_id, $request)) :
                    return $this->responseWithSuccess(__('parcel.collect_to_pickup_success'), [], 200);
                else :
                    return $this->responseWithError(__('parcel.error_msg'), [], 500);
                endif;
                break;
