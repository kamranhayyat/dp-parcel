...
class MerchantShopsController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        $cities = City::query()->select('id', 'name')->where('district_id', $edit_shop->district_id)->get();
        $singleMerchant = $this->repoMerchant->get($merchant_id);
        $districts = $this->repo->getAllDistricts();
        if (blank($singleMerchant) || blank($edit_shop)) {
            abort(404);
        }
        return view('backend.merchant.shops.edit', compact('edit_shop', 'merchant_id', 'singleMerchant', 'districts', 'cities'));
    }
