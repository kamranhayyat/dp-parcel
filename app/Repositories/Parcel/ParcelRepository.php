...
use App\Mail\DeliveryManAssign;
...
class ParcelRepository implements ParcelInterface
{
    ...
    {
        try {
            $collectToPickup                = new ParcelEvent();
            $collectToPickup->parcel_id     = $id;
            $collectToPickup->note          = $request->note;
            $collectToPickup->parcel_status = ParcelStatus::COLLECT_TO_PICKUP;
            $collectToPickup->created_by    = Auth::user()->id;
            if (isset($_FILES['image']['name']) && $_FILES['image']['name']) {
                ...
                $collectToPickup->delivered_image  = $delivered_image;
            }
            if (isset($_FILES['signatureImage']['name']) && $_FILES['signatureImage']['name']) {
                ...
                $collectToPickup->signature_image  = $signature_image;
            }
            $collectToPickup->save();
            $parcel                      = Parcel::find($id);
            $parcel->status              = ParcelStatus::COLLECT_TO_PICKUP;
            ...
            if ($request->send_sms_pickuman == 'on') {
                $msg = 'Dear ' . $collectToPickup->pickupman->user->name . ', Pickup Collected parcel with ID ' . $parcel->tracking_id . ' parcel from (' . $parcel->merchant->business_name . ',' . $parcel->merchant->user->mobile . ',' . $parcel->merchant->address . ') within ' . dateFormat($parcel->pickup_date) . ' -' . settings()->name;
                app(SmsService::class)->sendSms($collectToPickup->pickupman->user->mobile, $msg);
            }

            try {
                $msgNotification = 'Dear ' . $collectToPickup->pickupman->user->name . ', Pickup Collected parcel with ID ' . $parcel->tracking_id . ' parcel from (' . $parcel->merchant->business_name . ',' . $parcel->merchant->user->mobile . ',' . $parcel->merchant->address . ') within ' . dateFormat($parcel->pickup_date) . ' -' . settings()->name;
                app(PushNotificationService::class)->sendStatusPushNotificationTo($parcel, $collectToPickup->pickupman->user->device_token, $msgNotification, 'deliveryMan');
            } catch (\Exception $exception) {
            }


            if ($request->send_sms_merchant  == 'on') {
                $msg = 'Dear ' . $parcel->merchant->business_name . ', your  parcel with ID ' . $parcel->tracking_id . ' Pickup Collected ' . $collectToPickup->pickupman->user->name . ', ' . $collectToPickup->pickupman->user->mobile . ' Track here: ' . url('/') . ' -' . settings()->name;
                app(SmsService::class)->sendSms($parcel->merchant->user->mobile, $msg);
            }

            try {
                $msgNotification = 'Dear ' . $parcel->merchant->business_name . ', your  parcel with ID ' . $parcel->tracking_id . ' Pickup Collected ' . $collectToPickup->pickupman->user->name . ', ' . $collectToPickup->pickupman->user->mobile . ' Track here: ' . url('/') . ' -' . settings()->name;
                if (isset($parcel->merchant->user->web_token)) {
                    app(PushNotificationService::class)->sendStatusPushNotificationTo($parcel, $parcel->merchant->user->web_token, $msgNotification, 'merchant');
                }
                if (isset($parcel->merchant->user->device_token)) {
            ...
            foreach ($request->parcel_ids_ as $id) {
                ...
                if ($request->send_sms == 'on') {
                    ...
                    if (session()->has('locale') && session()->get('locale') == 'bn') :
                        ...
                        $msg = 'Dear ' . $parcel->customer_name . ', parcel with ID ' . $parcel->tracking_id . ' from (' . $parcel->merchant->business_name . ')' . settings()->currency . '(' . $parcel->cash_collection . ') delivery man assing by ' . $deliverymanAssign->deliveryMan->user->name . ', ' . $deliverymanAssign->deliveryMan->user->mobile . '. Track here:' . url('/') . '  -' . settings()->name;
                    endif;

                    $customer_sms_response =  app(SmsService::class)->sendSms($parcel->customer_phone, $msg);
                ...
                try {
                    ...
                    $data['user_type']          = UserType::MERCHANT;
                    Mail::to($parcel->merchant->user->email)->send(new DeliveryManAssign($data));
                    $data['user_type']          = UserType::DELIVERYMAN;
                    Mail::to($deliveryUser->user->email)->send(new DeliveryManAssign($data));
                } catch (\Exception $exception) {
                    // dd($exception);
                }
            }
    ...
    public function deliverymanAssign($id, $request)
    {
        ...
        try {
            ...
            $parcel                 = Parcel::find($id);
            $parcel->status         = ParcelStatus::DELIVERY_MAN_ASSIGN;
            $parcel->delivery_time  = $request->delivery_time;
            $parcel->delivery_type    = 1;
            $parcel->save();
            if ($request->send_sms == 'on') {
            ...
            try {
                ...
                Mail::to($parcel->merchant->user->email)->send(new DeliveryManAssign($data));
                $data['user_type']          = UserType::DELIVERYMAN;
                Mail::to($deliveyrman->user->email)->send(new DeliveryManAssign($data));
            } catch (\Exception $exception) {
                // dd($exception);
            }
            return true;
    ...
    {
        try {
            ParcelEvent::where('parcel_id', $id)
                ->whereIn('parcel_status', [parcelStatus::DELIVERY_MAN_ASSIGN, parcelStatus::DELIVERY_RE_SCHEDULE])
                ->delete();
    ...
    {
        try {
            $parcelEvent1                = new ParcelEvent();
            $parcelEvent1->parcel_id     = $id;
            // $parcelEvent1->note          = $request->note;
            $parcelEvent1->parcel_status = ParcelStatus::COLLECT_TO_PICKUP;
            $parcelEvent1->created_by    = Auth::user()->id;
            $parcelEvent1->save();

            $parcelEvent                = new ParcelEvent();
            $parcelEvent->parcel_id     = $id;
            $parcelEvent->note          = $request->note;
            $parcelEvent->parcel_status = ParcelStatus::BRANCH_CANCEL;
            $parcelEvent->created_by    = Auth::user()->id;
            $parcelEvent->save();
    ...
    public function parcelPartialDelivered($id, $request)
    {
        try {
            ...
            $total_charges                = ($cod_charges_amount             +
    ...
    public function deliverymanStatusParcel($status)
    {
        ...
        if ($status == ParcelStatus::DELIVERED) :
            return Parcel::orderBy('updated_at')
        ...
        elseif ($status == ParcelStatus::DELIVERY_MAN_ASSIGN) :
            return Parcel::orderBy('updated_at')
                ...
                ->with(['merchant'])
                ->whereIn('status', [
                    ParcelStatus::COLLECT_TO_PICKUP,
                ...
                ->where(function ($query) {
                    $query->wherehas('parcelEvent', function ($eventquery) {
                        $eventquery->where('delivery_man_id', Auth::user()->deliveryman->id);
                        $eventquery->orWhere('pickup_man_id', Auth::user()->deliveryman->id);
                    });
                })
                ->get();
        elseif ($status == ParcelStatus::PICKUP_ASSIGN) :
            return Parcel::orderBy('updated_at')
                ->orderBy('priority_type_id')
                ->with(['merchant'])
                ->whereIn('status', [ParcelStatus::PICKUP_ASSIGN, ParcelStatus::DELIVERY_RE_SCHEDULE])
                ->where(function ($query) {
                    $query->wherehas('parcelEvent', function ($eventquery) {
