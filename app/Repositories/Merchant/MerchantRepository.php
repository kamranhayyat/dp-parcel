...
class MerchantRepository implements MerchantInterface
{
    ...
    public function store($request)
    {
        ...
        try {
            ...
            $merchant->is_agreement        = $request->is_agreement;
            $merchant->target              = $request->target;
            $merchant->discount            = $request->discount;
            if ($request->agreement_file) :
                $merchant->agreement_file      = $this->merchaant_nid('', $request->agreement_file);
            endif;
    ...
    public function update($id, $request)
    {
        ...
        try {
            ...
            $merchant->discount            = $request->discount;
            if ($request->agreement_file) :
                $merchant->agreement_file      = $this->merchaant_nid($merchant->agreement_file, $request->agreement_file);
            endif;
    ...
    public function merchaant_nid($nid_id  = '', $image)
    {
        try {
            ...
            if (!blank($image)) {
                ...
                $nid             = date('YmdHis') . random_int(11111, 99999) . "." . $image->getClientOriginalExtension();
