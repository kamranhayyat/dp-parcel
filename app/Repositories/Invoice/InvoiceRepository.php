        ...
        try {
            ...
            if($strtotime <= $merchant_date && !$invoiceAlreadyGenerated):
                if(count($parcels) > 0){
                    ...
                    if(request()->process): 
                        if($merchantFind->is_agreement == 1):
                            $f  = Carbon::today()->subDays($merchantFind->payment_period)->startOfDay()->toDateTimeString();
                            $to = Carbon::today()->endOfDay()->toDateTimeString();
                            $merchant_parcelCount =  Parcel::where(function($query){
                                                        $query->where('status',ParcelStatus::DELIVERED);
                                                        $query->orWhere('partial_delivered',BooleanStatus::YES);
                                                    })->whereBetween('updated_at',[$f,$to])->count();
                            if($merchantFind->target != null && $merchantFind->target <= $merchant_parcelCount):
                                $invoice->discount_amount   = (($invoice->total_charge/100) * $merchantFind->discount);
                                $invoice->save();
                            endif;
                        endif; 
                    endif;
