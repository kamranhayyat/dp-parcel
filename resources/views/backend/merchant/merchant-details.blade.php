...
<ul class="list-group">
    ...
    <li class="list-group-item profile-list-group-item">
        <span class="float-left font-weight-bold">Agreement</span>
        <span class="float-right profile-list-group-item-addresss">
            @if ($singleMerchant->is_agreement)
                Target = {{ $singleMerchant->target }}<br/>
                Discount = {{ $singleMerchant->discount }}%<br/>
                File =  <a href="{{ $singleMerchant->agreement_prof }}" download="">Download</a> 
